...
class dr_parse_matcher():
    ...
    def tree_print(self, verbosity, tabs, output_file):
        ...
        if _config_args.get("parse_hw_resources") and (tbl_level != DR_ROOT_TBL_LEVEL):
            ...
            has_rules = dr_parse_rules(self, verbosity, tabs, output_file)
            if col_matcher is not None:
                _has_rules = dr_parse_rules(col_matcher, verbosity, tabs, output_file)
                has_rules = has_rules or _has_rules
            if not has_rules:
